...
// Menampilkan notifikasi di Android
void shell(const string& message) {
    string cmd = "cmd notifications -S bigtext -t '♨️ Angle Connector' 'Tag' '" + message + "' > /dev/null 2>&1";
    system(cmd.c_str());
}

// Menghapus semua konfigurasi terkait angle dan driver
void clearAngleSettings() {
    ...
    system("for key in $(cmd settings list global | grep angle | cut -f1 -d=); do cmd settings delete global \"$key\"; done");
    system("for key in $(cmd settings list global | grep driver | cut -f1 -d=); do cmd settings delete global \"$key\"; done");   
}

// Setup ANGLE Vulkan mode
